...
class BackEnd:
    def __init__(self, root):
        ...
        plotButton = tk.Button(root, text="Get Patient Report", command=self.PatientDischarger)
        canvas_main.create_window(300, 560, window=plotButton)

    # Working code
    def PatientDischarger(self):
        global pat_id, pat_doa

        self.patientid_var = self.patientid_entry.get()
        if self.patientid_var[0:10] == 'AIIMS/JDH/':
            pat_id = self.patientid_var.lstrip('AIIMS/JDH/')
        elif self.patientid_var[0:10] == 'aiims/jdh/':
            pat_id = self.patientid_var.lstrip('aiims/jdh/')
        else:
            pat_id = self.patientid_var
        print("Creating discharge report for:" +  pat_id)

        patid_label2 = tk.Label(root, text='Patient ID: AIIMS/JDH/' + str(pat_id))
        canvas_main.create_window(300, 610, window=patid_label2)

        self.doa_var = self.doa_entry.get()
        pat_doa = self.doa_var

        doa_label2 = tk.Label(root, text=str('DOA: ' + self.doa_var))
        canvas_main.create_window(300, 630, window=doa_label2)
        ...
        # URLs
        main_url = 'http://212.83.179.41/~aiimshis/patient_record/opd_pat_rpt.php?pat_id=AIIMS/JDH/' + pat_id
        ...
        class lft:
            ...
            def __init__(self):
                ...
                self.bt = find_val(r_test, 'Total')
                self.bd = find_val(r_test, 'Direct')
                self.bi = find_val(r_test, 'Indirect')
        ...
        folderpath = str(desktop) + '/Patient Reports'
        if not os.path.isdir(folderpath):
            os.makedirs(folderpath)
        filename = namer1.pname + '.docx'
        ...
        if os.path.isfile(filepath):
            ...
            canvas_main.create_window(300, 650, window=status_done_label)
            return
        if not os.path.isfile(filepath):
            status_done_label = tk.Label(root, text='Could not create patient report! Please try again...', bg='white',
                                         fg='red')
            canvas_main.create_window(300, 650, window=status_done_label)
            return
...
if __name__ == "__main__":
    ...
    canvas_main = tk.Canvas(root, width=600, height=680)
    canvas_main.pack()
